...
**						Aufruf des Hauptablaufs und der Stationsabl鋟fe
**
********************************************************************************************************
**
** Projekt            : BM
...
**                      04.06.2007      Christoph H黮smann  Funktionen umgeordnet wg. neuer Ordnerstruktur V2.00
**                      26.11.2007      Christoph H黮smann  Grundprg. V2.10 >> Zuf黨rung der Grundprogramme
**						09.01.2009		M.van de Wouw		躡erarbeitet f黵 Grundprogramm V2.2
...
// TODO: Headerdateien der Baugruppen einf黦en!
...
// ******************************
    ...
    E_Gateway_following_SPS_St302_FlexLink_Run  = Eing(A33,3);	//<-Flix Link run of the following SPS receive			//<- Zwischenf鰎derband l鋟ft von folgender SPS empfangen
...
//	frei								            = Eing(A33,15);
	...
	M_Gateway_St301_FlexLink_Run                = Eing(A34,3);	//<- Flix Link run of previous SPS receive			//<- Umlauff鰎derband l鋟ft von vorheriger SPS empfangen
	E_Gateway_before_SPS_St302_FlexLink_full	= Eing(A34,4);	//<- Connecting conveyor belt full of previous SPS receive	//<- Verbindungsf鰎derband voll von vorheriger SPS empfangen
...
// TODO: Eingaenge von der Kamera pr黤en
// Pipe-躡ergaben zum Kamerarechner 1
...
// ***********************************
	...
	SetAusg(A151,8,A_LED_Unload_Machine);          // LED f黵 Taster im Kabelkanal
	SetAusg(A151,9,A_LED_Bridge_StandByPosition);  // LED f黵 Taster im Kabelkanal
	SetAusg(A151,10,A_LED_Quit);               // LED f黵 Taster im Kabelkanal
...
//	SetAusg(A151,15,frei);
	...
	SetAusg(A152,3,M_Gateway_St301_FlexLink_Run);						// -> Enable Flex Link conveyor belt to the following SPS send	// -> Freigabe Flex Link Umlauff鰎derband an folgende SPS senden
	SetAusg(A152,4,E_St302_B1_FB_voll);									// -> WPC on connecting conveyor belt to the following SPS send	// -> Nest auf Zwischenf鰎derband an folgende SPSS senden
...
//	SetAusg(A153,15,frei);
	...
	SetAusg(A154,3,A_St302_A1_FlexLink_Enable);			  // ->	connecting conveyor belt to previous SPS send	// -> Zwischenf鰎derband l鋟ft an vorherige SPS senden
...
// ******************************
	SetAusg(A155,0,A_St301_A1_FlexLink_slow);	// +301-A1/E1   speed Flex Link conveyor belt		(Geschw. Umlauff鰎derband)
	SetAusg(A155,1,A_St301_A1_FlexLink_fast);	// +301-A1/E2   speed Flex Link conveyor belt		(Geschw. Umlauff鰎derband)
...
#ifdef Camera_Pipe_exist
// TODO: Ausgaenge zur Kamera pr黤en
// Pipe-躡ergaben zum Kamerarechner 1
	...
	// StartSignale der angegebenen Kameras werden zusammen 黚ergeben
	// 躡ergabe 1 => Startmerker Kameraschrittkette 1 (M.Start[K_StxKamera...]
	// 躡ergabe 2 => Startmerker Kameraschrittkette 2 (M.Start[K_StxKamera...]
	// 躡ergabe 3 => Startmerker Kameraschrittkette 3 (M.Start[K_StxKamera...]
	// 躡ergabe 4 => Startmerker Kameraschrittkette 4 (M.Start[K_StxKamera...]
	// 躡erbage 5 => Nr. des Kamera PC angeben
	// Wird eine NULL eingetragen so wird diese Kamera nicht ber點ksichtigt, der Start erfogt wie gehabt
...
/* TODO: Einstellungen des Scanners kontrollieren!
    躡ergabewert 1 (WORD) AxisNum                       // Axis Number													// AxisNum.
    躡ergabewert 2 (WORD) ErrorNum                     // Error Number													// FehlerBereich
    躡ergabewert 3 (WORD) COM                          // Serial interface												// Serielle Schnittstelle
    躡ergabewert 4 (WORD) Nestposition 				   // WPC Position where the scanner writh in the shift register	// Nestposition, wo der Scanner in das Schieberegister eintr鋑t
    躡ergabewert 5 (BOOL) nur_Nestnummer_eintragen     // only WPC Number , or shift register values writh?				// nur Nestnummer, oder auch Schieberegisterwerte eintragen?
    躡ergabewert 6 (BOOL) Nestfreigabe_abfragen        // WPC enable request or ignore									// Nestfreigabe abfragen oder ignorieren
    躡ergabewert 7 (bool) E_NestEingelaufen            // Input for WPC worn in the Axis								// Eingang f黵 Nest eingelaufen an der Achse
...
/* TODO: Einstellungen des Scanners kontrollieren!
    躡ergabewert 1 (WORD) AxisNum                       // Axis Number													// AxisNum.
    躡ergabewert 2 (WORD) ErrorNum                     // Error Number													// FehlerBereich
    躡ergabewert 3 (WORD) Nestposition 				   // WPC Position where the scanner writh in the shift register	// Nestposition, wo der Scanner in das Schieberegister eintr鋑t
    躡ergabewert 4 (BOOL) nur_Nestnummer_eintragen     // only WPC Number , or shift register values writh?				// nur Nestnummer, oder auch Schieberegisterwerte eintragen?
    躡ergabewert 5 (BOOL) Nestfreigabe_abfragen        // WPC enable request or ignore									// Nestfreigabe abfragen oder ignorieren
    躡ergabewert 6 (bool) E_NestEingelaufen            // Input for WPC worn in the Axis								// Eingang f黵 Nest eingelaufen an der Achse
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	{
		PC_Warning[0] = false;	// evtl. Stand By Warnung St.1 unterdr點ken

		DriveUnit[DriveUnit1].DU_ready_for_StandBy = true;
		HV_Kalibrierung(K_HV_Kalibrierung	// Schrittkette
						,DriveUnit1			// DriveUnitNum.
						,HV1				// Nr. der HV Pr黤ung
						,100				// Start des Zeitwertbereiches (5 Timer werden ben鰐igt)
						);
	}
#endif
...
// TODO: Variablen pr黤en
	WORD MaPartNum	= MaPart1;	// Machine Part Number for BDE						// Maschinenteil Nummer f黵 BDE angeben
...
// ---  und k鰊nen in der Fehler.txt durch Eingabe von
...
*/
	if (E_Key_311
	||  M_.without_311_Key		// ohne 311 Schl黶sel
...
/*	---------------------------------------------------------------------------------------
		...
		Erl鋟terung der Registereintr鋑e
		Es werden immer 5 stellige Nummern vergeben die sich wie folgt aufschl黶seln lassen
...
/*	---------------------------------------------------------------------------------------
	...
	Guteil / Pol				Maschinenteil			Station				Pr黤fehler
	...
	Ausschu遬ol	 MaTeil 3	 Station 7		Pr黤fehler 2
...
//--- Produktionsdaten an BDE 黚ergeben ---
//--- to transfer the production data to BDE ---
		...
		)
	{
		if (Index21_senden)
		{
			// ProDaten zwischenspeichern, alle Block & Polinfos auf einmal 黚ergeben
		...
		//Index 21 senden oder nur 22 (Pr黤fehler)
...
/* ------------------------------------------------------------------
/*
/* Parameter[0] : Anzahl Geh鋟se im Nest
/*
/*	1:	1 Geh鋟se im Nest
/*	2:	2 Geh鋟se im Nest
/*
...
void Machine_ON()
{
	if	(  (E_Button_MaOn && !E_Button_Quit)	// wenn beide gleichzeitg gedr點kt werden nicht einschalten!
		&&  E_Button_MaOff
		&& !M_.Newstart
		&& !M_.MaError
		&& !M_.MaStop
	...
	if	(	M_.MaOn
		...
		M_.MaRun = true;	// Maschine l鋟ft	// Maschine l鋟ft
...
void Machine_OFF(void)
{
	...
	// Maschine L鋟ft erst einen Zyklus sp鋞er ausschalten als M_.MaEin.
	// Damit haben die Stationen die m鰃lichkeit die Endstellung bei MaAus zu erreichen
	// (Abschnitt "Maschine l鋟ft AUS" vor "Maschine ein AUS" gezogen)
	...
	if  (   (  !M_.MaOn
    	...
    	||    M_.MaError									// Error Machine Stop								// St鰎ung -> Maschine sofort aus
        )
	{
		M_.MaRun = false;
...
void Error_MaOff(void)
{
	...
	// MaStop ausl鰏en auch, wenn die Maschine nicht l鋟ft.
	// Fehler die ohne Anlauf der Maschine entstehen k鰊nen angezeigt werden
	// Diese Fehler m黶sen in Allgemein eingetragen werden
...
//*****  ControlTime Machine On  ******
	...
	// Maschine h鋖t an wenn eine Kontrollzeit Abgelaufen ist
...
//**************************************
	...
	//Nach St鰎ung die Endstellung bei Start kontrollieren der Fehler ist vorrangig => die Luft wird ausgeschaltet
	...
	// 1.Zyklus Allgemeine St鰎ungen, Servo und Endstellungen nach Fehler suchen
...
void Unload_Machine(void)
{
	if  (   MACHINETYPE == FIRST_MG )
    {
        // Merker f黵 die Nester,die sich noch im Umlauf befinden,
        // und mit aufzuarbeitenden Geh鋟sen best點kt sind.
        ...
        ||  MACHINETYPE == LAST_MG )
    {
        ...
        // Maschine macht 8 Nestvorsch黚e,bevor sie leerf鋒rt
        // Bei kleiner Losgr鰏se k鰊nen sich Teile zwischen
...
// TODO: Wenn n鰐ig weitere Antriebe hinzuf黦en!!
...
void General_Error(void)
{
	...
	// Fehlerbereich f黵 den Allgemeinenteil festlegen
...
void EndPositionByStart()
{
	for (WORD i=0;i<NumOfDriveUnits;i++)
	{
		...
		// Endstellungsfehler beim Einschalten der Maschine bevor die Anl鋟ft
...
void Conveyor_OnOff(bool M_ConvReset, bool M_QuitReset)
{
	...
	if  ( PosFlanke( Fl_Conveyor, E_Button_ConveyorOn ) && !M_.MaRun )    // Push button Conveyor on/off	// Taster Vibratoren ein/aus bet鋞igen
    {
	...
	if  (   PosFlanke( Fl_QuitConveyor, M_.Quit )               // Button Quit		// Taster Entsperren
        ||  M_.MaRun )                                          // Machine Run		// Maschine l鋟ft
    {
		M_.ConveyorOn = true;
    }
...
void MachineProcess1(void)
{
	...
	// will be assigned									// w鋒rend die Achse f鋒rt kontrolliert
	ControlEndPosition();
	DriveUnitInEndPosition();

	// to query the switch-on condition for the variables	// Einschaltbedingungen f黵 die Variablen
	// "M_.MaOn" and "M_.MaRunning"							// "M_.MaEin" und "M_.MaLaeuft" werden abgefragt
	Machine_ON();

	// to query the switch-off condition for the variables	// Ausschaltbedingungen f黵 die Variablen
	// "M_.MaOn" and "M_.MaRunning"							// "M_.MaEin" und "M_.MaLaeuft" werden abgefragt
	Machine_OFF();
	...
	// M鰃lichkeit SPS seitig die Servicezeit im BDE zu aktivieren
	...
	// Protective door mistakes are formed				// Schutzt黵fehler werden gebildet
	// Error will look activated						// Fehler suchen wird aktiviert
	Error_MaOff();

	// to search for the prior and general errors					// vorrangige und allgemeine St鰎ungen werden zugewiesen
	// e.g. emergency stop, 3 times rejects, safety door errors		// z.B. Not-Aus, 3xAusschuss, Schutzt黵fehler
	General_Error();

	// to set the variable "M_.Quit",								// Die Variable "M_.Entsperren" wird gesetzt,
	// so that errors and other functions will be reset				// hiermit werden Fehler und andere Funktionen zur點kgesetzt
	Quit();
	...
	// to control the airpressure on the air distributor			// Abfrage des Druckluftw鋍hters am Luftverteiler
	Airpressure_control();

	// to assign the main air outputs										// Hauptluftausg鋘ge werden gesetzt
	// and control the end position if errors happen						// Endstellung bei "PosFlanke St鰎ung" kontrolliert
	...
	// to reset the main air outputs after errors					// Hauptluftausg鋘ge werden je nach St鰎ung zur點kgesetzt
	MainAir_Off();
	...
	// Transfer 1: Conveyor Reset = Set back the DelayTime from external 		// 躡ergabe 1: Vibratoren_Reset = Zur點ksetzen der Nachlaufzeit von Au遝n
	// TODO: ConvReset control / complete										// TODO: Vibr_Reset kontrollieren / erg鋘zen?
	...
	// to alarm light or bell according to the type of errors		// Leuchten und Hupen je nach St鰎ung zuweisen
	ErrorLight();
	MachineLightning(SBTMaPart1.E_SingleStepMode, false);

	// Startmerker der Pr黤stationen abfragen
	// Pr黤ungen fertig zuweisen
	// Ausschuss z鋒len durchf黨ren
	...
	// TODO: Check function				// TODO: Funktionen 黚erpr黤en
	...
	// set the status variables for BDE,										// Statusvariablen f黵 BDE zuweisen
	// calculate and set the machine data, e.g. clock cycle, good parts.		// Maschinendaten ermitteln und eintragen, z.B. Takte, Gutteile
	// Function in SPSProg.h													// Funktion in SPSProg.h
	BDE_Data_Transfer();

	// to call all the station processes										// Aufruf aller Stationsabl鋟fe
	...
	// to implement the instruction of BDE: delete the shift register		// Befehl von BDE ausf黨ren, Schieberegister der Maschine l鰏chen
	...
	// Through the variable "M_.MaStop und Allgemeiner_Fehler" 				// 躡er die Variablen "M_.MaStop und Allgemeiner_Fehler"
	...
	#ifndef Error_of_BDE_display				// no Error announcement about INDEX3 of BDE		// keine Fehleranzeige 黚er INDEX3 von BDE
				   true,						// Show Error by the internal functions 			// Fehler durch die internen Funktionen anzeigen
	#else
				   false,						// Error by BDE about INDEX 3 directly to the display program	//Fehler durch BDE 黚er INDEX 3 direkt an das Anzeigeprg.
	#endif
				   T_Error);					// timer for error of the text display				// Zeitwerk f黵 Fehler der Textanzeige

	// Error output over index 10											// Fehlerausgabe 黚er Index 10
	// Erros are sent via index 10											// Fehler die 黚er Index 10 gesendet werden
	...
	||  IFCBIT)
	{
		...
		#if defined(DoubleAxis) || defined(SingleAxis)
// TODO: Check Setting		Einstellungen pr黤en
			St201_CalculatePosition(DriveUnit1,Axis1);
		#endif
	}

	// to assign the states of output variables to			// Die Zust鋘de der Ausgangsvariablen
	// the address of bus termials							// werden den Adressen der Busteilnehmer zugewiesen
	Output();

	// Remanentevariable "M.Marker_ok" f黵 Speicherfehler aktivieren
	if (IFCBIT)
	{
	...
	// Startbit zur點ksetzen, nur im ersten Zyklus aktiv
	IFCBIT = false;
...
//*****************************************************************
//*****************  Hauptablauf 2  niedrige Priorit鋞  ***********
//*****************************************************************
