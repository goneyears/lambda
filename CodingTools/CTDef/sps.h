...
** Datum              : 01.01.2002
** Rev. Nr.		      : 0.1
** Rev. nderungen	  : siehe Verzeichnis "Standard_Prg" RevNr.xls
**
** Funktion			  : Eingangs- und Ausgangsadressierung, Grundfunktionen
**						wie Maschine Ein/Aus. Luft Ein/Aus etc.,
**						Aufruf des Hauptablaufs und der Stationsablufe
**
********************************************************************************************************
**
** Projekt            : BM 
...
** Programmierer      : 
**
** Programmnderungen : Datum			Name				Bemerkung
**						19.11.2005		Torsten Weber		Programmerstellung, mit Grundprogramm V1.26
**                      04.06.2007      Christoph Hlsmann  Funktionen umgeordnet wg. neuer Ordnerstruktur V2.00
**                      26.11.2007      Christoph Hlsmann  Grundprg. V2.10 >> Zufhrung der Grundprogramme
**						09.01.2009		M.van de Wouw		berarbeitet fr Grundprogramm V2.2
...
// TODO: Headerdateien der Baugruppen einfgen!
...
// TODO: Eingaenge von der Kamera prfen
...
// ***********************************
	...
	SetAusg(A151,8,A_LED_Unload_Machine);          // LED fr Taster im Kabelkanal
	SetAusg(A151,9,A_LED_Bridge_StandByPosition);  // LED fr Taster im Kabelkanal
	SetAusg(A151,10,A_LED_Quit);               // LED fr Taster im Kabelkanal
...
#ifdef Camera_Pipe_exist	
// TODO: Ausgaenge zur Kamera prfen
// Pipe-bergaben zum Kamerarechner 1
	...
	// Wird eine NULL eingetragen so wird diese Kamera nicht bercksichtigt, der Start erfogt wie gehabt
...
/* TODO: Einstellungen des Scanners kontrollieren!
    ...
    bergabewert 4 (WORD) Nestposition 				   // WPC Position where the scanner writh in the shift register	// Nestposition, wo der Scanner in das Schieberegister eintrgt
    ...
    bergabewert 7 (bool) E_NestEingelaufen            // Input for WPC worn in the Axis								// Eingang fr Nest eingelaufen an der Achse
...
/* TODO: Einstellungen des Scanners kontrollieren!
    bergabewert 1 (WORD) AxisNum                       // Axis Number													// AxisNum.
    bergabewert 2 (WORD) ErrorNum                     // Error Number													// FehlerBereich
    bergabewert 3 (WORD) Nestposition 				   // WPC Position where the scanner writh in the shift register	// Nestposition, wo der Scanner in das Schieberegister eintrgt
    ...
    bergabewert 6 (bool) E_NestEingelaufen            // Input for WPC worn in the Axis								// Eingang fr Nest eingelaufen an der Achse
...
St4_insert_pin_main		(  K_St4_MainProcess		// Step Pin insert				//Schrittkette Stift einsetzen
					...
					, 50					// Start 1 Error number for this funktion		//Start des 1.Fehlerbereichs fr diese Funktion
					, 50					// Start 2 Error number for this funktion		//Start des 2.Fehlerbereichs fr diese Funktion
					, 70					// Start 3 Error number for this funktion		//Start des 3.Fehlerbereichs fr diese Funktion
					, 80					// Start Error number for Festo PositionRad	//Start des Fehlerbereichs fr Festo Positionierrad
					, 90					// Start Error number for Festo	Ejector			//Start des Fehlerbereichs fr Festo Ausdruecker
					, 100					// Start Error number for Festo Inserter		//Start des Fehlerbereichs fr Festo Eindruecker
					, DriveUnit1			// AntriebsNr
              //      , Extra4				// Specialfunction for digital inputs			//Extrafunktion fr Digital Eingang
            //        , Extra5				// Specialfunction for digital inputs			//Extrafunktion fr Digital Eingang
					, 4						// Station in BDE								//StationsNr von BDE
					, 4						// registerposition for this station			//RegisterStelle fr die Station
...
St8_Stiftpruefung		( K_St8_pressure_test	// SchritteSEQ Spender
    	                , 130		            // Start des 1. Fehlerbereichs fr diese Funktion
	   	                , 130		            // Start des 1. Fehlerbereichs fr diese Funktion
						, 2     				//Air  Ventilinsel die bei Stoerung ausgeschaltet werden soll
			            , DriveUnit1              // DriveUnitNum
		            	, 8		                // StationsNr von BDE
			            , 8                     // RegisterStelle fr die Station
		            	, 730	                // Start des Fehlerbereichs fr die Servofunktion
						, Axis2                // AchsNr des Pruefers
						, Extra2                // Extrafunktion AnalogwertIn
						, Extra1                // Extrafunktion fr Digitalausgang
						, 0                     // benutztes Ausgangsbit der Ausgangskarte
						, 22030                 // Schieberegisterwert der Station fr nicht fertig bearbeitete Pole
												// register value of station for not finished pole
						, 12030	                // Grundwert fr Ausschussinformation im Register
						, 2010                  // Wert im Schieberegister fr GoodPole
						, 1                     // Prffehler: max. zul. Prfdruck berschritten
						, 2                     // Prffehler: Prfdruck nicht erreicht
						, 3                     // Prffehler: max. Ausdrckweg berschritten
						, 4                     // Prffehler: Kraft an falscher Position aufgetreten
...
//**********************************************************************************************************************************
	...
	{
		PC_Warning[0] = false;	// evtl. Stand By Warnung St.1 unterdrcken
		
		DriveUnit[DriveUnit1].DU_ready_for_StandBy = true;
		HV_Kalibrierung(K_HV_Kalibrierung	// Schrittkette
						,DriveUnit1			// DriveUnitNum.
						,HV1				// Nr. der HV Prfung
						,100				// Start des Zeitwertbereiches (5 Timer werden bentigt)
						);
	}
#endif
...
// TODO: Variablen prfen
	WORD MaPartNum	= MaPart1;	// Machine Part Number for BDE						// Maschinenteil Nummer fr BDE angeben
...
// ---  und knnen in der Fehler.txt durch Eingabe von
...
*/
	if (E_Key_311 
	||  M_.without_311_Key		// ohne 311 Schlssel
...
/*	---------------------------------------------------------------------------------------
		...
		Erluterung der Registereintrge
		Es werden immer 5 stellige Nummern vergeben die sich wie folgt aufschlsseln lassen
...
/*	---------------------------------------------------------------------------------------
	...
	Guteil / Pol				Maschinenteil			Station				Prffehler	
	...
	Ausschupol	 MaTeil 3	 Station 7		Prffehler 2
...
//--- to transfer the production data to BDE ---
		...
		)
	{
		...
		//Index 21 senden oder nur 22 (Prffehler)
...
/* ------------------------------------------------------------------
/*
/* Parameter[0] : Anzahl Gehuse im Nest
/*	
/*	1:	1 Gehuse im Nest
/*	2:	2 Gehuse im Nest
/*
...
void Machine_ON()
{
	if	(  (E_Button_MaOn && !E_Button_Quit)	// wenn beide gleichzeitg gedrckt werden nicht einschalten!
		&&  E_Button_MaOff
		&& !M_.Newstart
		&& !M_.MaError
		&& !M_.MaStop
	...
	if	(	M_.MaOn
		...
		M_.MaRun = true;	// Maschine luft	// Maschine luft
...
void Machine_OFF(void)
{
	...
	// Maschine Luft erst einen Zyklus spter ausschalten als M_.MaEin.
	// Damit haben die Stationen die mglichkeit die Endstellung bei MaAus zu erreichen
	// (Abschnitt "Maschine luft AUS" vor "Maschine ein AUS" gezogen)
	...
	if  (   (  !M_.MaOn 
    	...
    	||    M_.MaError									// Error Machine Stop								// Strung -> Maschine sofort aus
        )
	{
		M_.MaRun = false;
...
void Error_MaOff(void)
{
	...
	// MaStop auslsen auch, wenn die Maschine nicht luft.
	// Fehler die ohne Anlauf der Maschine entstehen knnen angezeigt werden
	// Diese Fehler mssen in Allgemein eingetragen werden
...
//*****  ControlTime Machine On  ******
	...
	// Maschine hlt an wenn eine Kontrollzeit Abgelaufen ist
...
//**************************************
	...
	//Nach Strung die Endstellung bei Start kontrollieren der Fehler ist vorrangig => die Luft wird ausgeschaltet
	...
	// 1.Zyklus Allgemeine Strungen, Servo und Endstellungen nach Fehler suchen
...
void Unload_Machine(void)
{
	if  (   MACHINETYPE == FIRST_MG )
    {
        // Merker fr die Nester,die sich noch im Umlauf befinden,
        // und mit aufzuarbeitenden Gehusen bestckt sind.
        ...
        ||  MACHINETYPE == LAST_MG )
    {
        ...
        // Maschine macht 8 Nestvorschbe,bevor sie leerfhrt
        // Bei kleiner Losgrsse knnen sich Teile zwischen
...
// TODO: Wenn ntig weitere Antriebe hinzufgen!!
...
void General_Error(void)
{
	...
	// Fehlerbereich fr den Allgemeinenteil festlegen
...
void EndPositionByStart()
{
	for (WORD i=0;i<NumOfDriveUnits;i++)
	{
		...
		// Endstellungsfehler beim Einschalten der Maschine bevor die Anluft
...
void Conveyor_OnOff(bool M_ConvReset, bool M_QuitReset)
{
	...
	if  ( PosFlanke( Fl_Conveyor, E_Button_ConveyorOn ) && !M_.MaRun )    // Push button Conveyor on/off	// Taster Vibratoren ein/aus bettigen
    {
	...
	if  (   PosFlanke( Fl_QuitConveyor, M_.Quit )               // Button Quit		// Taster Entsperren
        ||  M_.MaRun )                                          // Machine Run		// Maschine luft
    {
		M_.ConveyorOn = true;
    }
...
void MachineProcess1(void)
{
	...
	// will be assigned									// whrend die Achse fhrt kontrolliert
	ControlEndPosition();
	DriveUnitInEndPosition();

	// to query the switch-on condition for the variables	// Einschaltbedingungen fr die Variablen
	// "M_.MaOn" and "M_.MaRunning"							// "M_.MaEin" und "M_.MaLaeuft" werden abgefragt
	Machine_ON();

	// to query the switch-off condition for the variables	// Ausschaltbedingungen fr die Variablen
	// "M_.MaOn" and "M_.MaRunning"							// "M_.MaEin" und "M_.MaLaeuft" werden abgefragt
	Machine_OFF();
	...
	// Mglichkeit SPS seitig die Servicezeit im BDE zu aktivieren
	...
	// Protective door mistakes are formed				// Schutztrfehler werden gebildet
	// Error will look activated						// Fehler suchen wird aktiviert
	Error_MaOff();

	// to search for the prior and general errors					// vorrangige und allgemeine Strungen werden zugewiesen
	// e.g. emergency stop, 3 times rejects, safety door errors		// z.B. Not-Aus, 3xAusschuss, Schutztrfehler
	General_Error();

	// to set the variable "M_.Quit",								// Die Variable "M_.Entsperren" wird gesetzt,
	// so that errors and other functions will be reset				// hiermit werden Fehler und andere Funktionen zurckgesetzt
	Quit();
	...
	// to control the airpressure on the air distributor			// Abfrage des Druckluftwchters am Luftverteiler
	Airpressure_control();

	// to assign the main air outputs										// Hauptluftausgnge werden gesetzt
	// and control the end position if errors happen						// Endstellung bei "PosFlanke Strung" kontrolliert
	...
	// to reset the main air outputs after errors					// Hauptluftausgnge werden je nach Strung zurckgesetzt
	MainAir_Off();
	...
	// Transfer 1: Conveyor Reset = Set back the DelayTime from external 		// bergabe 1: Vibratoren_Reset = Zurcksetzen der Nachlaufzeit von Auen
	// TODO: ConvReset control / complete										// TODO: Vibr_Reset kontrollieren / ergnzen?
	...
	// to alarm light or bell according to the type of errors		// Leuchten und Hupen je nach Strung zuweisen
	ErrorLight();
	MachineLightning(SBTMaPart1.E_SingleStepMode, false);

	// Startmerker der Prfstationen abfragen
	// Prfungen fertig zuweisen
	// Ausschuss zhlen durchfhren
	...
	// TODO: Check function				// TODO: Funktionen berprfen
	...
	// set the status variables for BDE,										// Statusvariablen fr BDE zuweisen
	// calculate and set the machine data, e.g. clock cycle, good parts.		// Maschinendaten ermitteln und eintragen, z.B. Takte, Gutteile
	// Function in SPSProg.h													// Funktion in SPSProg.h
	BDE_Data_Transfer();

	// to call all the station processes										// Aufruf aller Stationsablufe
	...
	// to implement the instruction of BDE: delete the shift register		// Befehl von BDE ausfhren, Schieberegister der Maschine lschen
	...
	#endif
				   T_Error);					// timer for error of the text display				// Zeitwerk fr Fehler der Textanzeige
	...
	||  IFCBIT)
	{
		...
		#if defined(DoubleAxis) || defined(SingleAxis)
// TODO: Check Setting		Einstellungen prfen
			St201_CalculatePosition(DriveUnit1,Axis1);
		#endif
	}

	// to assign the states of output variables to			// Die Zustnde der Ausgangsvariablen
	// the address of bus termials							// werden den Adressen der Busteilnehmer zugewiesen
	Output();

	// Remanentevariable "M.Marker_ok" fr Speicherfehler aktivieren
	if (IFCBIT)
	{
	...
	// Startbit zurcksetzen, nur im ersten Zyklus aktiv
	IFCBIT = false;
...
//*****************************************************************
//*****************  Hauptablauf 2  niedrige Prioritt  ***********
//*****************************************************************
